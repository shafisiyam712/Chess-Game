...
function coloring() {
    ...
    color.forEach(color => {
        getId = color.id //get id of current box
        arr = Array.from(getId)//convert the id from string to array
        arr.shift()//remove the first index
        aside = eval(arr.pop())//take the last index and save it to aside
        aup = eval(arr.shift())//take the remaining first index and save it to aup
        a = aside + aup//put them together as a sum b801=>8+1=9

        if (a % 2 == 0) {
            // color.style.backgroundColor = 'rgb(232 235 239)'
            color.style.backgroundColor = 'rgb(219, 160, 85)'
        }
        if (a % 2 !== 0) {
            // color.style.backgroundColor = 'rgb(125 135 150)'
            color.style.backgroundColor = 'rgb(82, 39, 9)'
        }
    })
}
coloring()
